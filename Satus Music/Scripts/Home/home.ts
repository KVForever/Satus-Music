...
for (let i = 0; i < songs.items.length / 2; i++) { 
    ...
    var danceability = document.getElementById(`spot-${i + 1}-stats`).getElementsByClassName("progress-bar")[0] as HTMLElement;
    danceability.style.width = `${(features.danceability * 100)}%`
    var Energy = document.getElementById(`spot-${i + 1}-stats`).getElementsByClassName("progress-bar")[1] as HTMLElement;
    Energy.style.width = `${(features.energy * 100)}%`
    var Instrumentalness = document.getElementById(`spot-${i + 1}-stats`).getElementsByClassName("progress-bar")[2] as HTMLElement;
    Instrumentalness.style.width = `${(features.instrumentalness * 100)}%`
    var songName = document.getElementById(`spot-${i + 1}-stats`).getElementsByClassName("song-name")[0] as HTMLElement;
    songName.innerText = trackName
    var songArtist = document.getElementById(`spot-${i + 1}-stats`).getElementsByClassName("artist-name")[0] as HTMLElement;
...
function handleIntersect(entries, observer) {
    entries.forEach((entry) => {
        ...
        if (entry.intersectionRatio > prevRatio) {
            ...
            let width = increasingWidth.replace(
                "ratio",
                increase,
            );
...
for (let i = 0; i < 9; i++) {
    ...
    var songArtist = document.getElementById(`artist-spot-${i + 1}-stats`).getElementsByClassName("artist-name")[0] as HTMLElement;
    songArtist.innerText = artistName;
    var popularity = document.getElementById(`artist-spot-${i + 1}-stats`).getElementsByClassName("popularity")[0] as HTMLElement;
    popularity.innerText = artistPopularity;
    var followers = document.getElementById(`artist-spot-${i + 1}-stats`).getElementsByClassName("num-followers")[0] as HTMLElement;
    followers.innerText = artistFollowers;
    var genres = document.getElementById(`artist-spot-${i + 1}-stats`).getElementsByClassName("genres")[0] as HTMLElement;
...
function artistIntersect(entries, observer) {
    entries.forEach((entry) => {
        ...
        if (entry.intersectionRatio > artistPrevRatio) {
            ...
            element.classList.remove("reduce-card");
            let increase = (entry.intersectionRatio / 2 + 1).toString();
            let width = artistIncreasingWidth.replace(
                "ratio",
                increase,
            );
            element.style.width = width;
